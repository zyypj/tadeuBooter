...
public class Analyzer implements Opcode {
  ...
  private void mergeRet(IntQueue queue, CodeIterator iter, int pos, Frame frame, Subroutine subroutine) throws BadBytecode {
    ...
    for (Iterator<Integer> iterator = subroutine.callers().iterator(); iterator.hasNext(); ) {
      int caller = ((Integer)iterator.next()).intValue();
      ...
      for (Iterator<Integer> iterator1 = subroutine.accessed().iterator(); iterator1.hasNext(); ) {
