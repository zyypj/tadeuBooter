package me.syncwrld.booter.libs.javassist.bytecode.stackmap;
...
import me.syncwrld.booter.libs.javassist.bytecode.ByteArray;
...
public class MapMaker extends Tracer {
  ...
  public StackMap toStackMap2(ConstPool cp, TypedBlock[] blocks) {
    ...
    for (i = 1; i < n; i++) {
      TypedBlock bb = blocks[i];
      effective[i] = isTarget(bb, blocks[i - 1]);
      if (isTarget(bb, blocks[i - 1])) {
        bb.resetNumLocals();
        prev = bb;
        num++;
