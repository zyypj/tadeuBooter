...
final class Constants {
  ...
  static boolean isWhitelisted(String internalName) {
    if (!internalName.startsWith("me/syncwrld/booter/libs/asm/"))
      return false; 
    String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
    return (internalName.contains("Test$") || 
      Pattern.matches("me/syncwrld/booter/libs/asm/util/Trace" + member + "Visitor(\\$.*)?", internalName) || 
      
      Pattern.matches("me/syncwrld/booter/libs/asm/util/Check" + member + "Adapter(\\$.*)?", internalName));
  }
