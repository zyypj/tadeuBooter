package me.syncwrld.booter.libs.google.gson.stream;
...
public class JsonReader implements Closeable {
  ...
  public JsonReader(Reader in) {
    ...
    this.in = Objects.<Reader>requireNonNull(in, "in == null");
  }
  ...
  private int peekKeyword() throws IOException {
    ...
    if ((this.pos + length < this.limit || fillBuffer(length + 1)) && 
      isLiteral(this.buffer[this.pos + length]))
      return 0; 
    this.pos += length;
  ...
  private int peekNumber() throws IOException {
    ...
    if (last == 2 && j != 0 && (value != Long.MIN_VALUE || negative) && (value != 0L || !negative)) {
  ...
  public int nextInt() throws IOException {
    ...
    if (p == 16) {
      ...
      try {
        int i = Integer.parseInt(this.peekedString);
        this.peeked = 0;
        this.pathIndices[this.stackSize - 1] = this.pathIndices[this.stackSize - 1] + 1;
        return i;
      } catch (NumberFormatException numberFormatException) {}
  ...
  private boolean skipTo(String toFind) throws IOException {
    ...
    for (; this.pos + length <= this.limit || fillBuffer(length); this.pos++) {
    ...
    JsonReaderInternalAccess.INSTANCE = new JsonReaderInternalAccess() {
        public void promoteNameToValue(JsonReader reader) throws IOException {
          ...
          if (p == 13) {
            ...
            throw new IllegalStateException("Expected a name but was " + reader
                .peek() + reader.locationString());
          } 
        }
      };
  }
