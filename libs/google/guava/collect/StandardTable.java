    ...
    private class EntrySetIterator extends AbstractIterator<Map.Entry<R, V>> {
      ...
      protected Map.Entry<R, V> computeNext() {
        ...
        class EntryImpl extends AbstractMapEntry<R, V> {
          ...
          public V setValue(V value) {
            return NullnessCasts.uncheckedCastNullableTToT(((Map<C, V>)entry.getValue()).put(StandardTable.Column.this.columnKey, (V)Preconditions.checkNotNull(value)));
          }
        };
        return endOfData();
      }
      ...
      public boolean contains(@CheckForNull Object obj) {
        if (obj instanceof Map.Entry) {
          ...
          if (StandardTable.this.containsColumn(entry.getKey()))
            return ((Map)Objects.<Map>requireNonNull(StandardTable.ColumnMap.this.get(entry.getKey()))).equals(entry.getValue()); 
        } 
        return false;
      }
