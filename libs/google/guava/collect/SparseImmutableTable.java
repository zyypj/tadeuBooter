...
final class SparseImmutableTable<R, C, V> extends RegularImmutableTable<R, C, V> {
  ...
  SparseImmutableTable(ImmutableList<Table.Cell<R, C, V>> cellList, ImmutableSet<R> rowSpace, ImmutableSet<C> columnSpace) {
    ...
    for (UnmodifiableIterator<R> unmodifiableIterator = rowSpace.iterator(); unmodifiableIterator.hasNext(); ) {
      R row = unmodifiableIterator.next();
      rows.put(row, new LinkedHashMap<>());
    } 
    Map<C, Map<R, V>> columns = Maps.newLinkedHashMap();
    for (UnmodifiableIterator<C> unmodifiableIterator1 = columnSpace.iterator(); unmodifiableIterator1.hasNext(); ) {
      C col = unmodifiableIterator1.next();
    ...
    for (int i = 0; i < cellList.size(); i++) {
      ...
      cellRowIndices[i] = ((Integer)Objects.requireNonNull((T)rowIndex.get(rowKey))).intValue();
      Map<C, V> thisRow = Objects.<Map<C, V>>requireNonNull(rows.get(rowKey));
      cellColumnInRowIndices[i] = thisRow.size();
      V oldValue = thisRow.put(columnKey, value);
      checkNoDuplicate(rowKey, columnKey, oldValue, value);
      ((Map<R, V>)Objects.<Map<R, V>>requireNonNull(columns.get(columnKey))).put(rowKey, value);
    } 
  ...
  Object writeReplace() {
    ...
    int i = 0;
    for (UnmodifiableIterator<Table.Cell<R, C, V>> unmodifiableIterator = cellSet().iterator(); unmodifiableIterator.hasNext(); ) {
      Table.Cell<R, C, V> cell = unmodifiableIterator.next();
      cellColumnIndices[i++] = ((Integer)Objects.requireNonNull((T)columnKeyToIndex.get(cell.getColumnKey()))).intValue();
    } 
