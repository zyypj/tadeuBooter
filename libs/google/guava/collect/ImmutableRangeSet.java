...
public final class ImmutableRangeSet<C extends Comparable> extends AbstractRangeSet<C> implements Serializable {
  ...
  private final class AsSet extends ImmutableSortedSet<C> {
    ...
    public int size() {
      ...
      if (result == null) {
        long total = 0L;
        for (UnmodifiableIterator<Range<C>> unmodifiableIterator = ImmutableRangeSet.this.ranges.iterator(); unmodifiableIterator.hasNext(); ) {
          Range<C> range = unmodifiableIterator.next();
    ...
    int indexOf(@CheckForNull Object target) {
      if (contains(target)) {
        Comparable comparable = (Comparable)Objects.<Object>requireNonNull(target);
        long total = 0L;
        for (UnmodifiableIterator<Range<C>> unmodifiableIterator = ImmutableRangeSet.this.ranges.iterator(); unmodifiableIterator.hasNext(); ) {
          Range<C> range = unmodifiableIterator.next();
