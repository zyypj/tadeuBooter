...
import me.syncwrld.booter.libs.google.thirdparty.publicsuffix.PublicSuffixPatterns;
import me.syncwrld.booter.libs.google.thirdparty.publicsuffix.PublicSuffixType;
...
public final class InternetDomainName {
  ...
  private int publicSuffixIndex() {
    ...
    if (publicSuffixIndexLocal == -2)
      this
        .publicSuffixIndexCache = publicSuffixIndexLocal = findSuffixOfType(Optional.absent()); 
    ...
    for (int i = 0; i < partsSize; i++) {
      ...
      if (i > 0 && 
        matchesType(desiredType, 
          Optional.fromNullable(PublicSuffixPatterns.UNDER.get(ancestorName))))
        return i - 1; 
  ...
  private static final CharMatcher PART_CHAR_MATCHER = DIGIT_MATCHER
    .or(LETTER_MATCHER).or(DASH_MATCHER);
