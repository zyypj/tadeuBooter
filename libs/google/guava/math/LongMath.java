...
import me.syncwrld.booter.libs.google.guava.primitives.UnsignedLongs;
    ...
    switch (mode) {
      ...
      case HALF_EVEN:
        ...
        if (cmpRemToHalfDivisor == 0L) {
          increment = (mode == RoundingMode.HALF_UP || (mode == RoundingMode.HALF_EVEN && (div & 0x1L) != 0L));
          break;
        } 
  ...
  public static double roundToDouble(long x, RoundingMode mode) {
    ...
    switch (mode) {
      ...
      case HALF_EVEN:
        ...
        deltaToFloor = x - roundFloor;
        deltaToCeiling = roundCeiling - x;
        if (roundCeiling == Long.MAX_VALUE)
          deltaToCeiling++; 
        diff = Longs.compare(deltaToFloor, deltaToCeiling);
        if (diff < 0)
        ...
        switch (mode) {
          case HALF_EVEN:
            return ((DoubleUtils.getSignificand(roundFloorAsDouble) & 0x1L) == 0L) ? 
              roundFloorAsDouble : 
