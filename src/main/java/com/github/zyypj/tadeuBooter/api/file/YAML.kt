 ...
 */
class YAML @JvmOverloads @Throws(IOException::class, InvalidConfigurationException::class) constructor(
    name: String,
    ...
    fun saveDefaultConfig() {
        if (!configFile.exists()) {
            plugin.getResource(configFile.name)?.let {
                try { copyResource(configFile.name, configFile) }
                catch (e: IOException) { logError("Erro ao salvar recurso padrão para o arquivo ${configFile.name}", e) }
            } ?: run {
                try {
                    configFile.parentFile?.let { parent ->
                        if (!parent.exists() && !parent.mkdirs())
                            logError("Não foi possível criar diretório para o arquivo ${configFile.name}", IOException("mkdirs retornou false"))
                    }
                    if (!configFile.createNewFile())
                        logError("Não foi possível criar o arquivo ${configFile.name}", IOException("createNewFile retornou false"))
                } catch (e: IOException) {
    ...
    fun save() {
        try { save(configFile) }
        catch (e: IOException) { logError("Erro ao salvar o arquivo ${configFile.name}", e) }
    }

    /** Recarrega do disco. */
    fun reload() {
        try { loadConfig() }
        catch (e: Exception) { logError("Erro ao recarregar o arquivo ${configFile.name}", e) }
    }
