package me.zyypj.booter.minecraft.spigot.diagnostics.artificial.inteligence;
...
import me.zyypj.booter.minecraft.spigot.diagnostics.DiagnosticTools;
import me.zyypj.booter.minecraft.spigot.diagnostics.attack.AttackManager;
import me.zyypj.booter.minecraft.spigot.diagnostics.attack.AttackMethod;
import me.zyypj.booter.minecraft.spigot.diagnostics.attack.methods.*;
...
public class AdaptiveStressTestAI {
                ...
                .runTaskTimer(
                        ...
                        () -> {
                            ...
                            if (tps < TPS_LIMIT) {
                                Bukkit.getLogger()
                                        .warning(
                                                "[AdaptiveStressTestAI] ⚠️ TPS caiu para "
                                                        + tps
                                                        + "! Parando ataques...");
                                stopAttacks();
                                analyzeAndRecommend();
                            ...
                            if (entityCount > ENTITY_LIMIT) {
                                Bukkit.getLogger()
                                        .warning(
                                                "[AdaptiveStressTestAI] ⚠️ Muitas entidades carregadas ("
                                                        + entityCount
                                                        + "). Considere otimizar as configurações.");
                            }
    ...
    public static void setCGPTApi(String apiKey) {
        ...
        Bukkit.getLogger()
                .info("[AdaptiveStressTestAI] ✅ Chave da API do ChatGPT definida com sucesso!");
    }
    ...
    private static void executeSmartAttack() {
        ...
        if (attack != null) {
            Bukkit.getLogger()
                    .info(
                            "[AdaptiveStressTestAI] 🚀 Executando ataque: "
                                    + attack.getClass().getSimpleName()
                                    + " com intensidade "
                                    + attackIntensity);
                            ...
                            Bukkit.getScheduler()
                                    .runTask(
                                            ...
                                            () -> {
                                                Bukkit.getLogger()
                                                        .info(
                                                                "[AdaptiveStressTestAI] 🤖 Resposta da IA: \n"
                                                                        + recommendation);
                                            });
                        });
    }
    ...
    private static String getChatGPTRecommendation() {
        try {
            ...
            String prompt =
                    "O servidor Minecraft apresentou queda de TPS. Aqui estão as informações detalhadas:\n"
                            + "Configurações:\n"
                            + configs
                            + "\n"
                            + "Status do Servidor:\n"
    ...
    public static class ChatGPTClient {
        ...
        public static String sendMessage(String prompt) throws IOException {
            if (CHATGPT_API_KEY.isEmpty()) {
                Bukkit.getLogger()
                        .warning(
                                "[ChatGPTClient] ❌ Nenhuma chave de API definida! Use setCGPTApi().");
            ...
            while (attempt < MAX_RETRIES) {
                try {
                    ...
                    if (responseCode != 200) {
                        Bukkit.getLogger()
                                .warning("[ChatGPTClient] Erro na resposta: " + responseCode);
                        attempt++;
                        continue;
                    }
