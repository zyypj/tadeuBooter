package me.zyypj.booter.minecraft.spigot.location.util

import me.zyypj.booter.minecraft.spigot.location.geographic.SimpleLocation
...
object AreaUtils {
    ...
    fun getVerticalHeart(loc: Location, radius: Int): Collection<SimpleLocation> {
        ...
        val cz = loc.blockZ
        for (y in cy - radius..cy + radius) {
            val angle = acos((cy - y).toDouble() / radius)
            val offset = (radius * sin(angle)).roundToInt()
