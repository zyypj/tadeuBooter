...
/**
 * API de packet-based scoreboard para plugins Bukkit. Pode ser usada de forma segura de maneira
 * assíncrona, pois tudo é baseado em pacotes.
 *
 * <p>O projeto está disponível no <a href="https://github.com/zyypj/tadeuBooter">GitHub</a>.
 */
public abstract class ScoreBoardBase<T> {
    ...
    static {
        ...
        try {
            ...
            } else if (ScoreBoardReflection.nmsOptionalClass(null, "ScoreboardServer$Action")
                            .isPresent()
                    || ScoreBoardReflection.nmsOptionalClass(null, "ServerScoreboard$Method")
                            .isPresent()) {
                versionTemp = VersionType.V1_13;
            } else if (ScoreBoardReflection.nmsOptionalClass(
                                    null, "IScoreboardCriteria$EnumScoreboardHealthDisplay")
                            .isPresent()
                    || ScoreBoardReflection.nmsOptionalClass(null, "ObjectiveCriteria$RenderType")
                            .isPresent()) {
            ...
            if (ScoreBoardReflection.nmsOptionalClass(null, "ClientboundSetDisplayObjectivePacket")
                    .isPresent()) {
        ...
        try {
            ...
            Class<?> entityPlayerClass =
                    ScoreBoardReflection.nmsClass("server.level", "EntityPlayer", "ServerPlayer");
            Class<?> playerConnectionClass =
                    ScoreBoardReflection.nmsClass(
                            "server.network", "PlayerConnection", "ServerGamePacketListenerImpl");
            Class<?> packetClass = ScoreBoardReflection.nmsClass("network.protocol", "Packet");
            Class<?> packetSbObjClass =
                    ScoreBoardReflection.nmsClass(
                            gameProtocolPackage,
                            "PacketPlayOutScoreboardObjective",
                            "ClientboundSetObjectivePacket");
            Class<?> packetSbDisplayObjClass =
                    ScoreBoardReflection.nmsClass(
                            gameProtocolPackage,
                            "PacketPlayOutScoreboardDisplayObjective",
                            "ClientboundSetDisplayObjectivePacket");
            Class<?> packetSbScoreClass =
                    ScoreBoardReflection.nmsClass(
                            gameProtocolPackage,
                            "PacketPlayOutScoreboardScore",
                            "ClientboundSetScorePacket");
            Class<?> packetSbTeamClass =
                    ScoreBoardReflection.nmsClass(
                            gameProtocolPackage,
                            "PacketPlayOutScoreboardTeam",
                            "ClientboundSetPlayerTeamPacket");
            Class<?> sbTeamClass =
                    VERSION_TYPE.isHigherOrEqual(VersionType.V1_17)
                            ? ScoreBoardReflection.innerClass(
                                    packetSbTeamClass, inner -> !inner.isEnum())
                            : null;
            Field playerConnectionField =
                    Arrays.stream(entityPlayerClass.getFields())
                            .filter(
                                    field ->
                                            field.getType().isAssignableFrom(playerConnectionClass))
                            .findFirst()
                            .orElseThrow(NoSuchFieldException::new);
            Method sendPacketMethod =
                    Stream.concat(
                                    Arrays.stream(
                                            playerConnectionClass.getSuperclass().getMethods()),
                                    Arrays.stream(playerConnectionClass.getMethods()))
                            .filter(
                                    m ->
                                            m.getParameterCount() == 1
                                                    && m.getParameterTypes()[0] == packetClass)
                            .findFirst()
                            .orElseThrow(NoSuchMethodException::new);
            Optional<Class<?>> displaySlotEnum =
                    ScoreBoardReflection.nmsOptionalClass("world.scores", "DisplaySlot");
            CHAT_COMPONENT_CLASS =
                    ScoreBoardReflection.nmsClass(
                            "network.chat", "IChatBaseComponent", "Component");
            CHAT_FORMAT_ENUM =
                    ScoreBoardReflection.nmsClass(null, "EnumChatFormat", "ChatFormatting");
            ...
            SIDEBAR_DISPLAY_SLOT =
                    displaySlotEnum.isPresent()
                            ? ScoreBoardReflection.enumValueOf(DISPLAY_SLOT_TYPE, "SIDEBAR", 1)
                            : 1;
            PLAYER_GET_HANDLE =
                    lookup.findVirtual(
                            craftPlayerClass,
            ...
            PACKET_SB_DISPLAY_OBJ =
                    ScoreBoardReflection.findPacketConstructor(packetSbDisplayObjClass, lookup);

            Optional<Class<?>> numberFormat =
                    ScoreBoardReflection.nmsOptionalClass("network.chat.numbers", "NumberFormat");
            ...
            if (numberFormat.isPresent()) { // Versões 1.20.3+
                Class<?> blankFormatClass =
                        ScoreBoardReflection.nmsClass("network.chat.numbers", "BlankFormat");
                Class<?> fixedFormatClass =
                        ScoreBoardReflection.nmsClass("network.chat.numbers", "FixedFormat");
                Class<?> resetScoreClass =
                        ScoreBoardReflection.nmsClass(
                                gameProtocolPackage, "ClientboundResetScorePacket");
                MethodType scoreType =
                ...
                MethodType scoreTypeOptional =
                        MethodType.methodType(
                                ...
                                Optional.class,
                                Optional.class);
                MethodType removeScoreType =
                        MethodType.methodType(void.class, String.class, String.class);
                MethodType fixedFormatType =
                        MethodType.methodType(void.class, CHAT_COMPONENT_CLASS);
                Optional<Field> blankField =
                        Arrays.stream(blankFormatClass.getFields())
                                .filter(f -> f.getType() == blankFormatClass)
                                .findAny();
                Optional<MethodHandle> optionalScorePacket =
                        ScoreBoardReflection.optionalConstructor(
                                packetSbScoreClass, lookup, scoreTypeOptional);
                fixedFormatConstructor = lookup.findConstructor(fixedFormatClass, fixedFormatType);
                packetSbSetScore =
                        optionalScorePacket.orElseGet(
                                () -> {
                                    try {
                                        return lookup.findConstructor(
                                                packetSbScoreClass, scoreType);
                                    } catch (Throwable t) {
                ...
                Class<?> enumSbAction =
                        ScoreBoardReflection.nmsClass(
                                "server", "ScoreboardServer$Action", "ServerScoreboard$Method");
                MethodType scoreType =
                        MethodType.methodType(
                                void.class, enumSbAction, String.class, String.class, int.class);
                packetSbSetScore = lookup.findConstructor(packetSbScoreClass, scoreType);
            } else {
                packetSbSetScore =
                        lookup.findConstructor(
                                packetSbScoreClass, MethodType.methodType(void.class));
            }
            ...
            if (VERSION_TYPE.isHigherOrEqual(VersionType.V1_8)) {
                String enumSbActionClass =
                        VERSION_TYPE.isHigherOrEqual(VersionType.V1_13)
                ...
                ENUM_SB_ACTION =
                        ScoreBoardReflection.nmsClass(
                                "server", enumSbActionClass, "ServerScoreboard$Method");
                ENUM_SB_HEALTH_DISPLAY_INTEGER =
                        ScoreBoardReflection.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "INTEGER", 0);
                ENUM_SB_ACTION_CHANGE =
                        ScoreBoardReflection.enumValueOf(ENUM_SB_ACTION, "CHANGE", 0);
                ENUM_SB_ACTION_REMOVE =
                        ScoreBoardReflection.enumValueOf(ENUM_SB_ACTION, "REMOVE", 1);
            } else {
    ...
    @Getter private T title = emptyLine();
    ...
    public synchronized void updateLines(Collection<T> lines, Collection<T> scores) {
        ...
        if (scores != null && scores.size() != lines.size()) {
            throw new IllegalArgumentException(
                    "The size of the scores must match the size of the board");
        }
    ...
    /**
     * Define o valor de um campo no objeto alvo com base no tipo e ordem (count) dentre os campos
     * não-estáticos. Caso seja possível, utilize o ReflectionHelper se o nome do campo for
     * conhecido.
     */
    private void setField(Object target, Class<?> fieldType, Object value, int count)
        ...
        throw new NoSuchFieldException(
                "No field of type " + fieldType.getName() + " at count " + count);
    }

    /**
     * Define o valor de um campo cujo tipo é conhecido. Sobrecarga sem o parâmetro count (usa o
     * primeiro encontrado).
     */
    private void setField(Object target, Class<?> fieldType, Object value)
            throws ReflectiveOperationException {
        setField(target, fieldType, value, 0);
    }

    /**
     * Define um campo que representa um componente. Se a versão for inferior a 1.13, converte para
     * String usando serializeLine.
     */
    ...
    private void sendModernScorePacket(int score, ScoreboardAction action) throws Throwable {
        ...
        Object format =
                scoreFormat != null
                        ...
                        : BLANK_NUMBER_FORMAT;
        Object scorePacket =
                SCORE_OPTIONAL_COMPONENTS
                        ? PACKET_SB_SET_SCORE.invoke(
                                objName, this.id, score, Optional.empty(), Optional.of(format))
    ...
    private void checkLineNumber(int line, boolean checkInRange, boolean checkMax) {
        ...
        if (checkInRange && line >= this.lines.size()) {
            throw new IllegalArgumentException(
                    "Line number "
                            + line
                            + " is out of range (current board size: "
                            + this.lines.size()
                            + ")");
        }
